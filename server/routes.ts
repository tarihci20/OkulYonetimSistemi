...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/dashboard/current-day", isAuthenticated, async (req, res) => {
    try {
      // Gerçek sistem saatini doğrudan alalım
      const now = new Date();
  ...
  // Homework attendance batch endpoint
  app.post("/api/homework-attendance/batch", isAuthenticated, async (req, res) => {
    try {
      ...
      // Sonucu döndür
      res.json({
        message: "Yoklamalar başarıyla kaydedildi",
        created: result.created,
        updated: result.updated
      });
    } catch (error) {
      console.error("Toplu yoklama kaydederken hata:", error);
      res.status(500).json({ message: "Yoklamalar kaydedilirken bir hata oluştu" });
    }
  });
